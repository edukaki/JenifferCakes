...
export default function Navbar() {
  ...
  return (
    <nav className={`bg-white z-50 ${navScroll ? 'sticky top-0' : 'relative'}`}>
      <div className="relative flex flex-row w-full items-center justify-between p-4 2xl:max-w-[70%] 2xl:mx-auto">
        ...
        <ul
          ...
          } absolute top-full right-0 space-y-8 py-10 w-full bg-white text-center z-50 xl:relative xl:flex xl:flex-row xl:items-center xl:text-xl xl:space-y-0 xl:py-0 xl:w-auto xl:justify-center`}
        >
          {links.map(({ href, text }, index) => (
            <li key={index}>
              <Link
                href={href}
                className={`px-12 py-4 hover:bg-black hover:bg-opacity-20 transition-colors ease-in-out duration-300 xl:px-12 xl:py-12`}
